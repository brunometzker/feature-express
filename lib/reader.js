...
let readModelRecursively = (MODEL, propertyPath) => {
  if(Array.isArray(_.get(MODEL, propertyPath))) {
    console.log(propertyPath.replace(/\./g, ' > '));
  } else {
    console.log(propertyPath.replace(/\./g, ' > '));
    _.keys(_.get(MODEL, propertyPath)).forEach((property) => {
      readModelRecursively(MODEL, propertyPath + "." + property);
    });
  }
}
...
let buildModel = (fromPath) => {
  ...
  Object.keys(MODEL).forEach((key) => { readModelRecursively(MODEL, key); })
}
